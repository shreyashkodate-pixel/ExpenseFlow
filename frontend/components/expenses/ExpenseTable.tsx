...
export const ExpenseTable: React.FC<ExpenseTableProps> = ({
  ...
  return (
    <div className="glass-panel rounded-2xl overflow-hidden shadow-2xl flex flex-col border border-slate-800/80">
      ...
      <div className="block sm:hidden p-3 space-y-3">
        {loading ? (
          ...
          <div className="py-12 text-center text-slate-500">
            <Tag className="w-8 h-8 mx-auto mb-2 opacity-30 text-indigo-400" />
          ...
          expenses.map((expense) => (
            <div
              key={expense.id}
              className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800/80 shadow-md space-y-2.5"
            >
              <div className="flex items-start justify-between gap-2">
                ...
                <span className="text-base font-extrabold text-rose-400 font-mono shrink-0">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-800/60 text-xs">
                ...
                <div className="flex items-center space-x-1.5">
                  <span className="font-mono text-[10px] text-slate-400 mr-1 flex items-center gap-1">
            ...
            {loading ? (
              ...
              expenses.map((expense) => (
                <tr key={expense.id} className="hover:bg-slate-900/80 transition-colors group">
                  ...
                  <td className="py-3.5 px-4 sm:px-6">
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-bold bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 shadow-sm whitespace-nowrap">
                      <Tag className="w-3 h-3 mr-1.5 text-indigo-400" />
                      {expense.category.name}
                    </span>
                  </td>
                  <td className="py-3.5 px-4 sm:px-6 text-xs text-slate-300 font-mono whitespace-nowrap">
                  ...
                  <td className="py-3.5 px-4 sm:px-6 text-right font-extrabold text-rose-400 font-mono text-sm sm:text-base tracking-tight whitespace-nowrap">
                    -₹{expense.amount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                  </td>
      ...
      {/* Pagination Bar */}
      <div className="p-3.5 sm:p-4 bg-slate-950/80 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-400">
        <div>
          Showing page <span className="font-bold text-white">{page}</span> of{' '}
          <span className="font-bold text-white">{totalPages || 1}</span> ({totalItems} total items)
        </div>
