def test_expense_crud_and_search(client):
    ...
    # 3. Update expense
    put_res = client.put(f"/api/v1/expenses/{exp_id}", json={
        "description": "Updated Grocery Shopping",
        "amount": 1300.00
    })
    assert put_res.status_code == 200
    assert put_res.json()["description"] == "Updated Grocery Shopping"
    assert put_res.json()["amount"] == "1300.00"

    # 4. Search expenses
    search_res = client.get("/api/v1/expenses?search=Grocery")
    assert search_res.status_code == 200
    assert search_res.json()["total"] == 1
    assert search_res.json()["items"][0]["id"] == exp_id

    # 5. Filter expenses by date range
    date_res = client.get("/api/v1/expenses?date_from=2026-08-01&date_to=2026-08-31")
    assert date_res.status_code == 200
    assert date_res.json()["total"] == 1

    # 6. Export expenses as CSV
    export_csv = client.get("/api/v1/expenses/export?format=csv")
    ...
    export_pdf = client.get("/api/v1/expenses/export?format=pdf")
...
def test_expense_error_cases(client):
    # Invalid category ID
    res_inv_cat = client.post("/api/v1/expenses", json={
        "amount": 100.0,
        "category_id": 99999,
        "description": "Invalid",
        "date": "2026-08-27"
    ...
    res_404 = client.get("/api/v1/expenses/99999")
    assert res_404.status_code == 404
    assert res_404.json()["error_code"] == "EXPENSE_NOT_FOUND"

    # Invalid export format
    res_fmt = client.get("/api/v1/expenses/export?format=xml")
