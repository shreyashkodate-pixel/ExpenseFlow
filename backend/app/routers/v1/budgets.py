...
def get_budgets(
    ...
    """Fetch all budgets for the authenticated user, optionally filtered by month, year, or category."""
...
def get_budget_status(
    ...
    """Fetch real-time budget status and spending utilization percentage for the authenticated user."""
    now = datetime.now(timezone.utc)
    m = month if month is not None else now.month
    y = year if year is not None else now.year
    return budget_service.get_budget_status(db, month=m, year=y, user_id=current_user.id)
...
def create_or_update_budget(
    ...
    return budget_service.create_or_update_budget(db, payload, current_user.id)
...
def update_budget(
    ...
    return budget_service.update_budget(db, budget_id, payload, current_user.id)
...
def delete_budget(
    ...
    """Delete a budget goal verifying ownership."""
