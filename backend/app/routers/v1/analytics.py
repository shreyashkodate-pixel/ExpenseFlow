...
def get_daily_analytics(
    ...
    """Fetch daily total spending and category breakdown."""
    target_date = date if date is not None else datetime.now(timezone.utc).date()
    return analytics_service.get_daily_analytics(db, target_date)
...
def get_monthly_analytics(
    ...
    """Fetch monthly spending trend, daily breakdown, and category distribution."""
    now = datetime.now(timezone.utc)
    m = month if month is not None else now.month
    y = year if year is not None else now.year
    return analytics_service.get_monthly_analytics(db, month=m, year=y)
...
def get_yearly_analytics(
    ...
    """Fetch annual spending trends and month-by-month breakdown."""
    y = year if year is not None else datetime.now(timezone.utc).year
    return analytics_service.get_yearly_analytics(db, year=y)
...
def get_category_analytics(
    ...
    """Fetch category spending breakdown and percentage of total spend."""
    return analytics_service.get_category_analytics(db, month=month, year=year)
